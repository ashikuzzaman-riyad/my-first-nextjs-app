...
export default async function page() {
  ...
  return (
    ...
    <main className="min-h-screen bg-black container mx-auto p-10">
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
    {posts.map((item) => (
      <div
        key={item._id}
        className="bg-gray-900 rounded-2xl shadow-md hover:shadow-amber-400 hover:shadow-xl transition overflow-hidden"
      >
        {/* Product Image */}
        <div className="relative w-full h-72 bg-gray-700">
          <img
            src={item.image}
            alt={item.name}
          ...
          <div className="flex items-center mt-3 gap-2 text-yellow-400">
            {Array.from({ length: 5 }).map((_, i) => (
              <svg
                key={i}
                className={`w-5 h-5 ${
                  i < Math.round(item.rating) ? "fill-current" : "text-gray-600"
                }`}
